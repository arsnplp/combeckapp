...
export default function ClientLoginPage() {
  ...
  return (
    <div className="pt-8">
      {showPasswordBanner && (
        <div className="mb-4 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
          <p className="flex-1 text-[13px] text-amber-800">
            Créez un mot de passe pour sécuriser votre compte.{" "}
            <a href="/client/set-password" className="font-semibold underline">
              Configurer maintenant
            </a>
          </p>
          <button onClick={() => setShowPasswordBanner(false)} className="text-amber-600 hover:text-amber-800">
            <X className="h-4 w-4" />
          </button>
        </div>
      )}
      ...
      <div className="mb-8">
        <h1 className="text-[24px] font-bold text-gray-900 tracking-tight">Vos cartes de fidélité</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        >
          {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : (
            <>Voir mes cartes <ArrowRight className="h-4 w-4" /></>
          )}
        </button>
      </form>
