...
import {
  ...
  Link2, Wallet, CheckCircle2, Sparkles,
} from "lucide-react";
...
export default function AffiliateSignupPage() {
  ...
  return (
    <div className="pb-10">
      ...
      {/* ═══ HÉROS ═══ */}
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-b from-green-700 to-green-900 px-6 pb-8 pt-10 text-white sm:px-10">
        {/* halo décoratif */}
        ...
        <p className="cb-reveal inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/10 px-3 py-1 text-[11.5px] font-semibold uppercase tracking-wider text-green-100">
          <Sparkles className="h-3 w-3" /> Programme partenaires ComeBack
        </p>

        <h1 className={`${display.className} cb-reveal mt-4 text-[34px] font-800 leading-[1.05] sm:text-[42px]`}
        ...
        <a href="#inscription"
          className="cb-reveal mt-7 flex h-13 w-full items-center justify-center gap-2 rounded-2xl bg-white py-4 text-[15px] font-bold text-green-800 shadow-xl shadow-green-950/30 transition-all hover:bg-green-50 active:scale-[0.98] sm:w-auto sm:px-8"
          style={{ animationDelay: ".7s" }}>
          Devenir partenaire — c&apos;est gratuit <ArrowRight className="h-4 w-4" />
        </a>
        <p className="cb-reveal mt-3 text-[11.5px] text-green-100/60" style={{ animationDelay: ".8s" }}>
      ...
      <section className="mt-10">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {CIBLES.map((c, i) => (
            <span key={c} className="cb-reveal rounded-full border border-green-200 bg-green-50 px-3.5 py-1.5 text-[12.5px] font-medium text-green-800"
      ...
      {/* ═══ FORMULAIRE ═══ */}
      <section id="inscription" className="mt-10 scroll-mt-20">
        <div className="overflow-hidden rounded-3xl border border-green-900/[0.08] bg-white shadow-xl shadow-green-900/[0.06]">
          ...
          <form onSubmit={submit} className="space-y-3 p-6">
            ...
            <button type="submit" disabled={loading}
              className="flex h-13 w-full items-center justify-center gap-2 rounded-2xl bg-green-600 py-4 text-[15px] font-bold text-white shadow-lg shadow-green-600/25 transition-all hover:bg-green-700 active:scale-[0.98] disabled:opacity-50">
